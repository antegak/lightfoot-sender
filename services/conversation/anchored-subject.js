...
function resolveAnchoredSubject({
  ...
  if (profiles?.isFamily && has(query, /\u0431\u0440\u0435\u043d\u0434|\u043a\u0430\u043a\u0438\u0435\s+\u0432\u0437\u044f\u0442\u044c|\u0447\u0442\u043e\s+\u043b\u0443\u0447\u0448\u0435|\u043f\u043e\u0441\u043e\u0432\u0435\u0442/)) {
    return buildAnchor('family', 90, 'family_advice_request');
  }

  const carried = ageAnchor(previousAnchor);
  if (carried && isFollowUpMessage(query, parsedQuery)) {
    return { ...carried, confidence: Math.max(70, Number(carried.confidence || 0)), reason: 'anchored_follow_up' };
  }
