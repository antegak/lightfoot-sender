function asksAdvice(text = '') {
  return /какие|что\s+лучше|лучше\s+рассмотреть|посовет|подобрать|бренд|РєР°РєРёРµ|С‡С‚Рѕ\s+Р»СѓС‡С€Рµ|Р»СѓС‡С€Рµ\s+СЂР°СЃСЃРјРѕС‚СЂ|РїРѕСЃРѕРІРµС‚|Р±СЂРµРЅРґ/i.test(String(text || ''));
}

function asksConcreteProducts(text = '', parsed = {}) {
  return Boolean(
    /есть|налич|показ|вариант|черн|бел|цвет|размер|РµСЃС‚СЊ|РЅР°Р»РёС‡|РїРѕРєР°Р¶|РІР°СЂРёР°РЅС‚|С‡РµСЂРЅ|С†РІРµС‚|СЂР°Р·РјРµСЂ/i.test(String(text || ''))
    || parsed.colorCode
    || parsed.colorHuman
    || parsed.brand
    || parsed.size
